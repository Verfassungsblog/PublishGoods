...
pub fn list_persons(
    ...
    //Sort persons
    match order {
        OrderBy::FirstnameAscending | OrderBy::FirstnameDescending => {
            persons.sort_unstable_by(|a, b| a.first_names.cmp(&b.first_names))
        }
        OrderBy::LastnameAscending | OrderBy::LastnameDescending => {
            persons.sort_unstable_by(|a, b| a.last_names.cmp(&b.last_names))
        }
    }
    ...
    let selected_persons: Vec<&Person> = persons
        .iter()
