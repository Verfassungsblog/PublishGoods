...
impl PersonOrderBy {
    /// Fixed SQL fragment for this variant (not user input, so safe to splice into the query).
    fn as_sql(self) -> &'static str {
        match self {
            PersonOrderBy::FirstnameAscending => "first_names ASC",
            PersonOrderBy::FirstnameDescending => "first_names DESC",
            PersonOrderBy::LastnameAscending => "last_names ASC",
            PersonOrderBy::LastnameDescending => "last_names DESC",
        }
    }
}

/// Returns one page of persons, sorted and limited by the database (`ORDER BY` / `LIMIT` /
/// `OFFSET`) rather than in application code.
pub async fn list_paginated(
    pool: &PgPool,
    order_by: PersonOrderBy,
    offset: i64,
    limit: i64,
) -> Result<Vec<PersonV2>, DbError> {
    let query = format!(
        "SELECT id, first_names, last_names, orcid, gnd, ror FROM persons ORDER BY {} LIMIT $1 OFFSET $2",
        order_by.as_sql()
    );
    // Safe: `order_by.as_sql()` only ever yields one of the fixed strings in `PersonOrderBy::as_sql`,
    // never caller-supplied data.
    let people: Vec<PersonV2> = sqlx::query_as(sqlx::AssertSqlSafe(query))
        .bind(limit)
        .bind(offset)
        .fetch_all(pool)
        .await?;
...
pub async fn count(pool: &PgPool) -> Result<i64, DbError> {
    let count = sqlx::query_scalar!("SELECT COUNT(*) FROM persons")
        .fetch_one(pool)
        .await?;
