...
/// Rolling window used both to count recent failed attempts and to lock the account.
...
pub async fn record_failed_attempt(pool: &PgPool, user_id: Uuid) -> Result<i64, DbError> {
    sqlx::query!(
...
mod tests {
    ...
    async fn lockout_flow(pool: PgPool) -> sqlx::Result<()> {
        ...
        for _ in 0..MAX_LOGIN_ATTEMPTS {
            last_count = record_failed_attempt(&pool, user.id).await.unwrap();
        }
