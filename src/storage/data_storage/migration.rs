...
pub fn load_inner_data_storage(
    mut file: File,
    mut version: u64,
    path_to_data: &str,
    ...
    if version != CURRENT_VERSION {
        info!(
            "Migrating InnerDataStorage from v{} to latest version (v{}).",
    ...
    if version == 1 {
        v1_data = Some(bincode::decode_from_std_read::<InnerDataStorageV1, _, _>(
            &mut file,
        ...
        if Path::exists(Path::new(&format!("{}/templates", path_to_data))) {
            fs::rename(
                format!("{}/templates", path_to_data),
                format!("{}/templates-old", path_to_data),
            )?;
    ...
    if version == 2 {
        v2_data = if let Some(v1_data) = v1_data {
            ...
            Some(bincode::decode_from_std_read::<InnerDataStorageV2, _, _>(
                &mut file,
                bincode::config::standard(),
            )?)
        };
    ...
    if version == 3 {
        v3_data = if let Some(v2_data) = v2_data {
            ...
            Some(bincode::decode_from_std_read::<InnerDataStorageV3, _, _>(
                &mut file,
                bincode::config::standard(),
            )?)
        }
        ...
        InnerDataStorageV3 {
            ...
            persons: value
                ...
                .map(|(k, v)| {
                    (
                        k,
                        Arc::new(RwLock::new(PersonV2::from(v.read().unwrap().clone()))),
                    )
                })
                .collect(),
