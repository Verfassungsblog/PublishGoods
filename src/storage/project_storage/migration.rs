...
pub fn load_project_data(mut file: File, mut version: u64) -> Result<ProjectData, ProjectStorageError>{
    if version != CURRENT_VERSION {
        info!("Migrating ProjectData from v{} to latest version (v{}).", version, CURRENT_VERSION);
    }

    let mut v1_data: Option<OldProjectData> = None;
    if version == 1{
        v1_data = Some(bincode::decode_from_std_read::<OldProjectData, _, _>(&mut file, bincode::config::standard())?);
        version = 2;
    }
    ...
    if version == 2{
        v2_data = if let Some(v1_data) = v1_data {
            ...
            Some(bincode::decode_from_std_read::<ProjectDataV2, _, _>(&mut file, bincode::config::standard())?)
        };
        version = 3;
    }
    ...
    if version == 3{
        v3_data = if let Some(v2_data) = v2_data {
            ...
            Some(bincode::decode_from_std_read::<ProjectDataV3, _, _>(&mut file, bincode::config::standard())?)
        };
        version = 4;
    }
    ...
    if version == 4{
        v4_data = if let Some(v3_data) = v3_data {
            ...
            Some(bincode::decode_from_std_read::<ProjectDataV4, _, _>(&mut file, bincode::config::standard())?)
        };
        version = 5;
    }
    ...
    if version == 5{
        v5_data = if let Some(v4_data) = v4_data {
            ...
            Some(bincode::decode_from_std_read::<ProjectDataV5, _, _>(&mut file, bincode::config::standard())?)
        };
        version = 6;
    }
    ...
    if version == 6{
        v6_data = if let Some(v5_data) = v5_data {
            ...
            Some(bincode::decode_from_std_read::<ProjectDataV6, _, _>(&mut file, bincode::config::standard())?)
        };
        version = 7;
    }
